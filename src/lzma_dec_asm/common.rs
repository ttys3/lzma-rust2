//! 7-Zip's hand-written LZMA decoder (`LzmaDec_DecodeReal_3`) and the state
//! it operates on, shared by the arm64 and x86-64 builds.
//!
//! The assembly itself lives in the sibling architecture module (`arm64.rs`,
//! `x86_64.rs`), which embeds the preprocessed copy from `src/asm/` with
//! `global_asm!` and provides `decode_real()`. Both routines implement the
//! symbol decoding loop of 7-Zip's C decoder and are tightly coupled to that
//! decoder's data model: the [`CLzmaDec`] struct layout, the flat probability
//! array layout and the calling contract below are all dictated by the
//! assembly and must not change. (The x86-64 code addresses the probability
//! array through `probs_1664`, the arm64 code through `probs`; the Rust side
//! always fills in both.)
//!
...
//!
//! Both routines save every callee-saved register they use on their own
//! frame, use no globals and call nothing, so they are reentrant and
//! thread-safe.

use alloc::vec::Vec;
use core::ptr;

use super::arch::decode_real;
use crate::{
...
pub(crate) struct CLzmaDec {
    ...
    /// `probs + 1664`: the x86-64 code addresses the array through this
    /// pointer with negative displacements, the arm64 code through `probs`.
...
// The struct offsets the assembly uses (`.equ offset_*` on arm64, the
// `CLzmaDec_Asm` struct on x86-64), verified at compile time.
const _: () = {
