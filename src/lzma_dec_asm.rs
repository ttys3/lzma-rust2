...
cfg_lzma_asm! {
    if {
        ...
        //
        // This must be a static rather than an empty function: LLVM can fold
        // identical functions and make an inlined `asm!` operand name another
        // function while `global_asm!` still defines the original symbol.
        #[used]
        static DECODE_REAL_SYMBOL_PREFIX: u8 = 0;
